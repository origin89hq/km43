...
| A `wrq` replayed to hold a session past its 15-minute expiry | A | The session expires on schedule and the replay gets no answer: P-077 refreshes only on a verified request P-022 did not refuse, and a replayed `req_id` is refused silently |
