...
boot, and written ahead of `execute`.
...
**Two trees, two counts.** The protocol and the controller are separate repositories, and each keeps its own `traceability.toml` over the same specification. The count in this one is what the wire crate's tests stand behind; the controller's obligations, which are most of the `P-` rules about sessions, enrolment and concerns and every `L-` rule, are tested in origin89. Once the controller builds against this repository, its copy of this check walks this tree beside its own crates and counts both. Neither number is the whole truth on its own, and the one that says *every requirement has a test somewhere* is the controller's, because that is where the two halves meet.
...
| No arithmetic in the `seq`/`req_id`/tick path overflows | `checked_`/`saturating_` is a rule; a proof is what says every path took it |
...
**Framing.** Round trip every length 0..`MAX_FRAME` × four fill patterns (already in `crates/xtask/src/vectors.rs` self-check — move it into the crate too, so both the generator and the implementation carry it). The Cheshire & Baker examples as known vectors, including 254-then-zero, where an encoder and a decoder wrong the same way agree with each other and with nobody else. **Every single-bit flip in a CRC-framed message is caught by the CRC, and every one in a sealed message by the tag** (conformance item 3) — 8 × len flips per message, cheap, and it is the test that says the CRC is not a false witness. An unauthenticated message on a transport without the CRC, a `Discover` over WebSocket, has neither, and a flipped bit there is a different valid message; it is tested against its own parsing rules, not this property. Two frames run together are rejected, not concatenated.
...
**Sealed body.** Exactly key 1 on a request and keys 1 and 2 on a response or an event (P-231); P-013 switched off; a duplicate integer key rejected before anything in the map is interpreted. Then the one that is worth more than a test: **type-state.** `Sealed` has no accessor for the plaintext; `Sealed::open` is the only way to an `Opened`, which has one. P-051's verify-before-decode is then not a rule an implementer can get wrong, it is a program that does not compile. That is the seam where the CLAUDE.md type-state preference genuinely earns itself. The sender's rule gets the same treatment: the nonce lives in the sealing state and moves on every seal, so there is no parameter a caller could pass twice (P-232).

**Tags and nonces.** The vectors are the known-answer half: the HKDF outputs, the fingerprint, both HMAC tags, and both handshakes message by message. The property half is three sentences. Every domain-separation label in P-043's table is distinct and none is a prefix of another, and each derivation computed under its own label disagrees with the same inputs under every other label — which is what says `km43/v1/pair-refused` and `km43/v1/hello-admit` are not two spellings. (Not "for all inputs the outputs differ": a fixed-width output over a larger input space collides somewhere, and a property that asserts otherwise fails on a correct implementation.) For every sequence of seals, no nonce is issued twice under one key, and a session ends before the counter would wrap. For every byte of a sealed message and of its associated data, flipping it makes the message fail to open. Truncation is leftmost-16 and that is a vector, not a property. The admission tag is compared through `subtle::ConstantTimeEq`; constant time is not provable here, so it is a review item and it goes in `traceability.toml` as `judgement`.

**Session state.** A bounded action-sequence property over `{Discover, Pair, Enrol, Hello, sealed, signed, Goodbye, expire, ClientDisconnected, comms_reboot, reset}` with invariants rather than expected outputs: at most one challenge per connection row at any moment (P-060); a challenge is consumed exactly once (P-061); at most one pairing handshake per connection (P-229); no draw from the controller's random bit generator is ever returned twice, whichever step a reset lands on (P-237); an accepted `req_id` is never accepted again. The invariant form matters — a property that asserts an expected output is a unit test with a generator in front of it.
...
| `sealed_open` | arbitrary `(key, bytes)` | never opens on a tag mismatch, and never decodes an inner body that did not open; a nonce at or below the window's floor is refused before the tag is checked and moves nothing (P-233) |
...
**This exact divergence is what stage 0b prevents**, and it is the first exchange of every session, so it is what the first interop run hits. Put the pre-session transcript first in the corpus regardless — `Discover`, `Pair`, `Enrol`, `Hello` before any sealed exchange — because a stage that fails on message one has told you something and a stage that fails on message forty has told you less.

**Also first, from the findings:** a `Hello` whose response was dropped, followed by `Discover` and a second `Hello` on the same bound row. LINK.md does not say what a second success does, P-076 covers only `Hello`-after-`Goodbye`, and the client that lost its response holds no session keys at all, because `Hello 0x81` carries the handshake message that completes them, so it cannot even send `Goodbye`. It is reachable from the most ordinary transient the threat model explicitly accepts. Two implementations will fill it two different ways, which is the P-104 shape exactly.
...
|---|---|---|
| After the tag verifies, before the dedup entry persists | the command did not execute; the client's retry under a new `req_id` succeeds | P-080's stated fail-closed |
| After the dedup entry persists, before execute | the retry meets the entry in flight and is answered from the state store, so the command runs at most once | P-080's in-flight rule |
| Mid-`execute`, before dedup complete | answer from the state store, not from the entry | the case P-121 is about |
| **Between epoch increment and client-table clear** | no key enrolled before the reset opens a session after it, wherever the cut lands | P-085 now makes the epoch write the reset's single commit, and P-239 frees every slot from an earlier epoch, so a cut before the write leaves the unit as it was and a cut after it leaves only dead keys. This row found the hole that rule closes: in v1 the reset had no fail-closed rule, and a cut here re-minted a byte-identical `client_key` into slot 1 — the failure P-085 quoted as its own reason for existing, produced by the remedy. It is the reason the stage is a loop and not a list: nobody put it on a list |
...
**FRAM write failure (not a crash — a refusal).** P-079's exit when the dedup entry will not persist: error 7, class A `0x0501`, and the command MUST NOT execute. Extend the same treatment to P-085's epoch write, and test both, because a write that fails is a fault this corpus already accepts as real rather than a speculative one.
...
- **A — the comms processor.** Assumed compromised. May drop, delay, reorder, replay, hold CTS, invent connections, offer times, read every envelope and every `Discover`, and stamp `session_id`. Reads no sealed body.
- **B — an enrolled client whose key leaked.** A stolen phone, a compromised relay. Holds a real client private key, the controller key it pinned, and a mask fixed at enrolment.
- **C — somebody who photographed the label.** Can derive `pair_psk` and `refusal_key` and nothing else; needs the button to enrol, and a network position during somebody else's pairing to try to sit in the middle of it.
...
|---|---|---|
| A response moved to another outstanding `req_id` fails to open | A | P-047, conformance 8 |
| A live `session_id` stamped onto a different connection's frame fails | A | the handle is in the prologue and `session_id` in every associated data string (P-072, P-234) |
| **A signed write withheld for hours, delivered after later requests on the same session, is refused before it acts** | A | **Fails today.** Nothing measures a request's age; nothing in a write's body proves recency; no client-side timeout is specified anywhere. This test is the artefact that drives the receiver-side `req_id` rule, and it is free now because `Command` is reserved |
...
| A sealed read request replayed to hold a session past its 15-minute expiry | A | The session expires on schedule and the replay gets no answer: P-077 refreshes only on a verified request P-022 did not refuse, and a replayed `req_id` is refused silently |
| **A signed frame captured before a P-078 reclaim, replayed after it** | B | Found here as a spec hole: the reclaim set the counter to 0 and unbound nothing, so the still-bound session read the row the reclaim had just zeroed. P-240 now unbinds every session on the slot and erases its key before the slot is rewritten, and a frame sealed under an old session cannot open under a new one (P-078). The test proves an implementation does both. The live exposure is a `SetConfig` section nobody else edited; the reserved exposure is `Command` |
| A cloud client (mask bit 3 clear) sends `Time 0x0A` | B | `TimeAck` outcome 3, inside the sealed response, never an `Error` (P-105, P-141) |
...
| A `Pair` whose message 1 opens, with no window open | C | outcome 2 under the refusal tag and no message 2 (P-241) — never a bare error, or a forged refusal sends somebody back to press a button that was never needed |
| A holder of the label answers the owner's `Discover` during a pairing and runs `XXpsk0` with the owner's phone | C | the phone abandons at message 2, before message 3, because the static key does not match the label's fingerprint (P-236, conformance 16) |
| Eight `Pair`s whose message 1 does not open, in sixty seconds | A | connection closed, reason 3, and the row and its challenge go with it. The question this row used to say was open — does a failed pairing count? — P-051 answers: a message 1 or 3 that does not open counts, and a `window_closed` or `table_full` refusal does not, because it is answered only after the label's key opened message 1 and counting it would let anybody holding the label close a technician's connection. The count is on the connection row |
| Seven `Pair`s that do not open, a `Goodbye` and a fresh `Hello`, then one more | A | still closed. A threshold the peer resets by handshaking again is not a threshold, and this is the one case a per-session counter passes every other test and fails |
| A client key enrolled before a factory reset, presented after it | B | refused with error 12: a slot from an earlier epoch is free and its key matches nothing (P-085, P-239). Paired with stage 6's epoch-write row, which is the same property from the other side |
...
- **The physical button** — and this is the one to say out loud. P-066 defines a *120-second window opened by the pushbutton or selector gesture*, with a temporary first-enrolment power-on exception on a board without a pushbutton, surfaced as `pairing_open`. Firmware must prove that boot reads a valid empty client table, including after factory reset or recovery of a lost or corrupt table; the boot that repairs an absent or unreadable table opens nothing, and a later boot that reads the valid empty table is eligible; P-116 requires the contact *held* at the instant a `Time 0x0A` is processed; DEFERRED entry 6 adds a third gesture for a firmware downgrade; factory reset is a fourth use of the same word. No host test can distinguish a window from a hold, because on the host they are the same boolean. Until the gestures are distinguished in prose, the bench cannot prove anything about them either — so this is a stage-0 dependency wearing a hardware hat.
...
4. Cut power during a FRAM write a thousand times overnight on a USB relay, assert every record reads back as its old value or its new one.
...
- eight `Pair`s whose message 1 does not open close the connection; eight `window_closed` refusals do not;
