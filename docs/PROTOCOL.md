...
wrapper MAC or signed-body MAC verified on that session. Outbound MUST NOT
...
7. Every table reaches its cap under load and refuses, and none evicts — except
   ...
   count.
8. A replayed request is rejected by the counter; a replayed event is rejected by
   `seq`; a `LogPage` whose entries go backwards is accepted; a response moved to
   another `req_id` fails its MAC.
