...
which only a relay reordering them ever tests. The last six bound who the client
table holds and what an invite may cost, and each is answered by an outcome
inside a sealed response, which tells the client everything it could do with
the number.
...
Five tags on this wire are HMACs, and all are 128 bits for the reason the
...
on has ended. An invite's proof and confirmation (P-257) are HMACs because the
invitee and the controller have no session between them, only a relay.
...
| Messages | Sealed by |
|---|---|
| Requests `0x03`, `0x05`, `0x06`, `0x07`, `0x08`, `0x09`, `0x0A`, `0x0C`, `0x0D`, `0x0E`, `0x0F`, `0x10`, `0x11`, `0x12`, `0x14`, `0x15`, `0x16`, `0x17`, `0x18` | the client |
| Responses `0x83`, `0x85`, `0x86`, `0x87`, `0x88`, `0x89`, `0x8A`, `0x8C`, `0x8D`, `0x8E`, `0x8F`, `0x90`, `0x91`, `0x92`, `0x94`, `0x95`, `0x96`, `0x97`, `0x98`, and `0xFF` in its sealed form (P-142) | the controller |
...
**P-053** — Signed requests (`0x07`, `0x08`, `0x09`, `0x0A`, `0x15`, `0x16`,
`0x17`) are sealed like every other request, and their inner body is their
operation body (P-080).
...
the `client_kind` shown beside it (P-105). The pre-shared key
...
whatever the owner who approved it chose.

**No message raises a mask and no message lowers one**, because no message
changes a role. A mask any client can edit is a mask the most exposed client
edits first.
...
Every **write** is signed: configuration, firmware, time, commands, and every
change to who is enrolled. *Signed* names the list and not a second layer. A
write is sealed like every request, under keys only a handshake with that
client's static key produces, and its inner body is its operation body, nothing
wrapped around it.
...
## Clients, roles and invites

Several people look after one site, and most of them never stand at its panel.
The label enrols whoever is standing there; everybody else is invited by
...
who pairs at the panel after that is an admin, because standing at the panel
with the label proves presence and nothing about who the site belongs to. An
owner is made only by an owner, through an invite that names the new key. A role
no message changes is a role a compromised admin cannot raise, and one no
...
The cloud's identity is a `viewer`. It reads readings, inventory, concerns,
history and events, because raising an alarm is what it is for, and writes
nothing. It does not read the network section, the scan list or the client
list: an SSID and the names of everybody who manages a cabin are not something
...
ClientRow
  1: client_id    u32
  2: generation   u32
  3: role         u8       see REGISTRY
  4: client_kind  u8       see REGISTRY; shown, decides nothing (P-105)
...
Both lists fit one response: eight slots and `MAX_INVITES` invites, each at its
...
client sets (P-004): measured on the wall clock, an invite nobody approved could
be kept live for a year by writing the time backwards. It is in RAM for the same
reason a pairing handshake is (P-229): a reboot is the one event every
unattended controller has, and an invite that does not survive one is an invite
that cannot outlive the person who proposed it by more than the next power cut.
The owner's row is the reason the table can refuse at all: without it, two
...
failure is error 7 `busy`, nothing is stored, and the controller MUST raise a
class A `concern raised` (`0x0501`) at condition `client table write failed`. The budget
MUST be restored to `INVITE_BUDGET` when an owner approves an invite this slot
proposed, and when the slot is written for a new enrolment (P-239). Nothing else
restores it: not a reboot, not a decline, not an expiry. `owner` slots have no
budget.
...
is a budget a power cut restores, and a site that loses power every night gives
the attacker a fresh one every morning. A record of its own because it is written
on every proposal, and a brown-out during one must tear at most the budget,
never the key record every session on that slot depends on.

### Approve — `0x16` / `0x96`

```text
operation body of Approve  0x16
...
2. the sender may make this decision, or outcome 3 `unauthorised`, and the
   invite stays pending. An owner may approve or decline; the invite's own
...
consuming the invite: otherwise any admin could spend an owner's pending
approvals by guessing nothing more than a nonce the client list shows it.
Step 4 is the inviter's authority checked at commit rather than at proposal,
because an owner approving an admin's invite an hour after that admin was
removed would otherwise enrol the removed admin's choice. Steps 7 and 8 leave
the invite pending because nothing was wrong with it and the owner can free a
...
```text
transcript = device_id[16] | CS[32] | epoch:u32 | suite:u8 | role:u8
           | inviter:u32 | inviter_generation:u32 | IS[32] | N_c[16] | N_i[16]

commitment = SHA-256("km43/v1/invite-commit" | IS | N_i)
proof      = HMAC(admit_key, "km43/v1/invite-proof" | transcript)[0..16]
confirm    = HMAC(admit_key, "km43/v1/invite-confirm" | transcript
                  | client_id:u32 | generation:u32)[0..16]
digits     = u32be(SHA-256("km43/v1/invite-sas" | transcript)[0..4]) mod 1000000
```

`admit_key` is P-238's for the invitee's key and the controller's, and `suite`
is the inviter's slot's. The invitee MUST draw `N_i` and its key from its own
CSPRNG, MUST NOT reveal `N_i` before it holds `N_c`, and MUST refuse a
controller key that makes the agreement zero (P-228). The digits MUST be shown
as six digits with leading zeros. An owner MUST NOT approve, and an invitee
MUST NOT keep an enrolment (P-222), until a person has said the digits on both
screens match, compared over a channel the relay does not carry: in person or
on a call, never in a message the app relayed. The invitee MUST verify
`confirm` before it keeps anything, and one that does not verify is an
enrolment it does not have.

The relay carries everything between the invitee and the controller, so it can
hand the invitee a key of its own for the controller and hand the controller a
key of its own for the invitee. The proof closes the first: it is under the
agreement with the real controller key, so an invitee told the relay's key
fails at step 5. Only a person closes the second, because nothing the invitee
sends can prove which key the owner meant to invite. The digits are over the
...
match and that received no confirmation MUST NOT assume it was refused. It
keeps its key and the controller key, and a `Hello` that succeeds names its
slot (keys 30 and 31); error 12 says it has none.

### Remove — `0x17` / `0x97`

```text
operation body of Remove  0x17
  1: client_id    u32      the slot
  2: generation   u32      the enrolment in it (P-239)

RemoveAck  0x97         sealed
  1: outcome      u8       see REGISTRY
```
...
A slot that cannot be freed durably is answered outcome 5 `not_stored`, and the
controller MUST raise a class A `concern raised` (`0x0501`) at condition
`client table write failed`. The sessions and invites step 4 ended stay ended.

Nothing is acknowledged before the generation has moved, because the generation
is what every other record keyed by `client_id` tells enrolments apart by, and
a removal acknowledged first is one a power cut can leave undone with the person
who asked believing it done. Removal names a generation so a retried `Remove`
that arrives after the slot was given to somebody else removes nobody. Owner
slots are not removable by any message: the one attack this whole section has
to survive is a compromised admin phone, and the first thing it would do is
...
table the admins and the cloud have filled is still one an owner can be invited
into without a reset, and the admins alone cannot fill the cloud's row.

### Client events

A change to who is enrolled is a class A record in the log, carried in
`Event 0x04` key 4, so an owner whose phone was elsewhere learns of every key
that was added or taken away, and who did it.

```text
ClientEnrolled  0x0603
  1: client_id    u32
  2: generation   u32
  3: role         u8
  4: client_kind  u8
  5: label        text     <= MAX_LABEL
  6: approver     u32      optional; the owner that approved an invite. Absent
                           for an enrolment through the label
  7: approver_generation  u32   present exactly when key 6 is
  8: inviter      u32      present exactly when key 6 is
  9: inviter_generation   u32   present exactly when key 6 is

ClientRemoved  0x0605
  1: client_id    u32
  2: generation   u32      the enrolment that ended
  3: role         u8
  4: by           u32      the slot that sent Remove 0x17
  5: by_generation  u32

InviteProposed  0x0606
  1: role         u8
  2: inviter      u32
  3: inviter_generation  u32
  4: client_kind  u8
  5: label        text     <= MAX_LABEL
```

A reclaim (P-240) is a `client enrolled` for the slot's new generation; the
enrolment it replaced ends without a `client removed`, because nobody removed it.
