...
2. For a `Command 0x08`, look up `(client_id, cmd_id)` in the dedup table, with
   the `client_id` the session is bound to. A match left **in flight** by a reset
   is answered from the state store, below; any other match is answered per
   P-120 and P-124, **without executing**.
